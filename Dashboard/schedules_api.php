<?php
// schedules_api.php - GET (list), POST (insert), DELETE (remove)
...
if ($method === 'GET') {
  $stmt = $pdo->query("SELECT id, title, DATE_FORMAT(date_start, '%Y-%m-%d') AS date_start, 
    ...
    FROM schedules ORDER BY date_start DESC");
...
if ($method === 'POST') {
  ...
  $stmt = $pdo->prepare("INSERT INTO schedules (title, date_start, date_end) VALUES (?, ?, ?)");
